...
class TestProcessToolCall:
    ...
    def test_process_tool_call_weather(self):
        ...
        result = function_calling.process_tool_call("get_weather", {"city": "Berlin"})
        assert "Sonnig" in result or "22" in result

    def test_process_tool_call_unknown_tool(self):
        """Test: Unbekanntes Tool"""
        result = function_calling.process_tool_call("unknown_tool", {})
...
class TestToolDefinitions:
    ...
    def test_weather_tool_definition(self):
        ...
        weather_tool = next(
            t for t in function_calling.tools if t["name"] == "get_weather"
        )
